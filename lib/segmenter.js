 ...
 */
function processSegment(s3Bucket, s3Dir, file, contentType, fps, framesPerSegment, segmenter) {
    ...
    downloadToLocal(videoUri, localVideoPath, function (err, contentType) {
      ...
      tmp.dir({unsafeCleanup: true}, function (err, segmentsPath, cleanupSegmentsDir) {
        ...
        extractSegments(localVideoPath, format, segmentsPath, function (err, files) {
          ...
          // getting metadata for the first segment
          getVideoMetadata(files[0], function (err, metadata) {
            if (err) return handleError(err);

            var fps = metadata.video.fps;

            var durationPattern = /^(\d+):(\d+):(\d+(?:\.\d+)?)$/;
            var durationMatches = durationPattern.exec(metadata.duration.raw);
            var framesPerSegment = Math.ceil(fps * durationMatches[3]);

            // processing segments
            files.forEach(function (file) {
              segmentTasks.push(processSegment(s3Bucket, s3Dir, file, contentType, fps, framesPerSegment, segmenter));
            });
